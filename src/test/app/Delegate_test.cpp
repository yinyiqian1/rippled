...
class Delegate_test : public beast::unit_test::Suite
{
    ...
    testPaymentGranular(FeatureBitset features)
    {
        ...
        // PaymentMint/PaymentBurn must follow the trustline balance direction.
        // Endpoint identity and trust limits are not enough to determine whether
        // DirectStepI will issue or redeem IOUs. These cases cover trustlines
        // whose limits point one way, but whose current balance makes execution
        // take the opposite path.
        {
            ...
            // even though the endpoint identity matches.
            {
                ...
                env(trust(gw, alice["USD"](200)));
                env.close();

                // Alice issues her own USD to gw; now gw holds alice's IOUs.
                env(pay(alice, gw, alice["USD"](100)));
                env.close();
        ...
        // correctly. AccountSet permissions don't apply to Payment and vice versa
        {
            ...
            env.close();
            env(pay(gw, alice, usd(50)), delegate::As(bob));
            env.require(Balance(alice, usd(150)));
        }
    }
